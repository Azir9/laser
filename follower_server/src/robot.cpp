...
void Robot::Pub_Speed()
{
    ...
    //以距离为阈值给出线速度
    float flw_dist = sqrt((last_x-transform.getOrigin().x())*(last_x-transform.getOrigin().x())
     + (last_y-transform.getOrigin().y())*(last_y-transform.getOrigin().y()));
    float diff_dist;
    if((last_x-transform.getOrigin().x())>0){
     diff_dist = flw_dist - keep_dist;//安全距离也可以改动
    }
    else{
      diff_dist = -flw_dist + keep_dist;//安全距离也可以改动

    }
    float flw_linear = diff_dist * 0.8;
    ...
    //给出角速度
    float d_angle = 0;
    float abs_x = std::fabs((last_x-transform.getOrigin().x()));
    // float abs_x =(last_x-transform.getOrigin().x());

    double rol,pit,yaw;
    tf::Matrix3x3(transform.getRotation()).getRPY(rol,pit,yaw);
    if(abs_x != 0) d_angle = atan((last_y-transform.getOrigin().y())/(last_x-transform.getOrigin().x()))-yaw;
