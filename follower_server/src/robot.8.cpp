...
min_first(0.2),max_first(3.0),
first_data_sucess(false),
max_cluster_dis(0.05),min_cluster_num(5),cluster_min_max_dis(0.3),
max_Leg_range(8),
...
keep_dist(1.0),
max_angular_vel(0.6),max_linear_vel(0.55),start_count(0),stop_count(0),count(0),update_count(0),openni_start(0),openni_stop(1),follow_finish(0),
max_curve(1.4),min_curve(1.06),noise_ax(0),noise_ay(0),laser_dt(0.1),update_n(4)
  {  
    //获取参数
...
std::vector<People> Robot::LegToPeople(std::vector<Leg_cluster> vLeg_Cluster)
{
  ...
  for(auto it= vLeg_Cluster.begin(), ite= vLeg_Cluster.end(); it!=ite; ++it)
  {
    ...
    it->dis_last = distence_(*it,x_(0),x_(1));
...
//记录观测值
    z_<< people_.x,people_.y;
      ...
      x_[2]=(people_.x - last_x)/laser_dt;
      x_[3]=(people_.y - last_y)/laser_dt;
    ...
    Kalman_Update(z_);
      ...
      //过几个周期更新速度
      if(update_n<update_count)
      {
        Vel_Acc_Update(update_count);
        update_count=0;
      }
      x_[2]=predict_vel_x;
      x_[3]=predict_vel_y;
...
void Robot::Kalman_Init(VectorXd &x_in, MatrixXd &P_in, MatrixXd &F_in,
...
void Robot::Vel_Acc_Update(int &update_n)
{
  ...
  noise_ax= 4*(ss_x-predict_vel_x)/update_n/update_n/laser_dt/laser_dt;
  noise_ay= 4*(ss_y-predict_vel_y)/update_n/update_n/laser_dt/laser_dt; 
  predict_vel_x=ss_x/update_n/laser_dt;
  predict_vel_y=ss_y/update_n/laser_dt;
