...
max_cluster_dis(0.04),min_cluster_num(4),cluster_min_max_dis(0.3),
max_Leg_range(8),
...
max_curve(1.4),min_curve(1.06),update_n(3),first_scan(1),use_diff(false),diff_num(3)
  {  
    //获取参数
...
void Robot::Frame_Diff(const sensor_msgs::LaserScan::ConstPtr& scan){
  for(int i = 0;i<scan_len;i++){
    ...
    diff_data[i]=std::fabs(scan_data.ranges[i]-last_scan_data.ranges[i]);
    //两帧雷达数据小于一定阈值则认定为背景点
    if(diff_data[i]<0.015)
    {
