...
min_first(0.2),max_first(4.0),
first_data_sucess(false),
max_cluster_dis(0.07),min_cluster_num(3),cluster_min_max_dis(0.25),
max_Leg_range(10),
...
keep_dist(1.0),
max_angular_vel(0.6),max_linear_vel(0.55),start_count(0),stop_count(0),count(0),openni_start(0),openni_stop(1),follow_finish(0),max_curve(1.4),min_curve(1.05)
  {  
    //获取参数
...
/*1.统计聚类数据*/
  ...
  for(int i=0;i<len;++i)
  {
    ...
    Leg_cluster_.avg_y = all_y/(Leg_cluster_.end_index - Leg_cluster_.start_index +1);//这一类点的平均Y
        ...
        //计算聚类总长
    for(int aa=Leg_cluster_.start_index+2;aa<Leg_cluster_.end_index-2;aa++)
    {
      	      float temp_x,temp_y;
...
/*2.Leg条件判断*/
    ...
    bool cluster_curve = (Leg_cluster_.avg_dis > min_curve && Leg_cluster_.avg_dis < max_curve);//类里面的平均距离应小于max_Leg_range
    
    if(enough_point_ && min_max_dis_ && cluster_avg_dis && line_length && cluster_curve)
    {
      //printf("dis_:%f\n",dis_);
