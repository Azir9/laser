...
keep_dist(1.0),
max_angular_vel(0.6),max_linear_vel(0.55),count(0)
  {  
    //获取参数
    ...
    */
    ...
    openni_client = n_.serviceClient<openni_tracker_test::SetFollowState>("/openni_follow");
...
void Robot::scan_callback(const sensor_msgs::LaserScan::ConstPtr& scan)  
  { 
    ...
    std::vector<People> vPeople_Cluster = LegToPeople(vLeg_Cluster);  //人腿位置放在vPeople_Cluster向量里
    		printf("vPeople_Cluster : %d\n",vPeople_Cluster.size());
...
void Robot::Pub_Speed()
{
    ...
    float flw_linear = diff_dist * 0.4;
    ...
    if(abs_x != 0) d_angle = atan(people_.y/people_.x);
