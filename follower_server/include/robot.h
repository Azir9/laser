...
/*******************leg detecter***********************/
/*******************KalmanFilter***********************/
void Kalman_Init(Eigen::VectorXd &x_in, Eigen::MatrixXd &P_in, Eigen::MatrixXd &F_in,
      Eigen::MatrixXd &H_in, Eigen::MatrixXd &R_in, Eigen::MatrixXd &Q_in);

void Kalman_Update(const Eigen::VectorXd &z);

void Kalman_Predict();

void Vel_Acc_Update(int &update_n);


/*******************KalmanFilter***********************/
/*******************frame_diff***********************/

void Frame_Diff(const sensor_msgs::LaserScan::ConstPtr& scan);
...
/*******************leg detecter***********************/
 ...
 //laser data
  ...
  float max_refind;//第一帧人距离激光的最大距离
  ...
  float last_y;
  float last_per_x;//上一周期人的xy坐标
  float last_per_y;

  bool set_use_predict_vel;//是否使用预测速度来跟踪
  int get_predict_vel;//使用预测速度预测当前帧的位置
  ...
  float success_num =0;
  float frame_num =0; 
  bool montionless_flag= false;
  int lost_count =0;
  // //雷达角度分辨率
  // float angle_increment= 0.004363;
  // int cluster_num;

  //laser scan time
  double laser_dt;
  double dt_4=laser_dt*laser_dt*laser_dt*laser_dt;
  double dt_3=laser_dt*laser_dt*laser_dt;
  double dt_2=laser_dt*laser_dt;
